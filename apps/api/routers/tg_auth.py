"""
VitaForge API - Telegram auth routes
"""
...
from models import MemberStreak, User
...
async def get_or_create_tg_user(
        ...
        created = sb.auth.admin.create_user(
            {
                ...
                "email_confirm": True,
                "user_metadata": {"telegram_id": telegram_id, "username": username or ""},
            }
        )
        uid = uuid.UUID(created.user.id)
    except Exception:
        try:
            users_page = sb.auth.admin.list_users()
            users = getattr(users_page, "users", users_page)
            uid = next(uuid.UUID(item.id) for item in users if getattr(item, "email", None) == phantom)
        except StopIteration as exc:
            raise HTTPException(500, "Telegram user yaratishda xatolik") from exc

    existing_user_q = await db.execute(select(User).where(User.id == uid))
    new_user = existing_user_q.scalar_one_or_none()
    if new_user:
        new_user.telegram_id = telegram_id
        new_user.full_name = f"{first_name} {last_name or ''}".strip()
        new_user.avatar_url = photo_url
        new_user.role = role
    ...
    streak_q = await db.execute(select(MemberStreak).where(MemberStreak.member_id == uid))
    if streak_q.scalar_one_or_none() is None:
        db.add(MemberStreak(member_id=uid, current_streak=0, longest_streak=0, total_points=0, badges=[]))
...
async def tg_miniapp_auth(body: TgMiniAppAuth, response: Response, db: AsyncSession = Depends(get_db)):
...
async def tg_widget_auth(body: TgWidgetAuth, response: Response, db: AsyncSession = Depends(get_db)):
